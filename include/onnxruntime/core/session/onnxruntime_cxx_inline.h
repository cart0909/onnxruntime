...
inline Session::Session(Env& env, const void* model_data, size_t model_data_length,
  ...
  ThrowOnError(Global<void>::api_.CreateSessionWithExternalDataFromArray(env, model_data, model_data_length, external_data_names_.data(), external_data_buffers, external_data_len, options, &p_));
}
